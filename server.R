...
shinyServer(function(input,output,session){
  ...
  output$out1 <-renderTable({
    ...
    if(input$F2==1){
      forecast1<-ses(InsampleTs,h=length(OutsampleTs),level = input$CI)
      forecast_out1 <- forecast1$lower
    }
    if(input$F2==2){
      forecast1<-ses(InsampleTs,h=length(OutsampleTs),initial = "simple",level = input$CI,alpha = input$AlphaS)
      forecast_out1 <- forecast1$lower
    }
    (data.frame(Y = as.character(as.matrix(forecast_out1)), date = as.character(as.Date(forecast_out1))))
  })
  ...
  output$out2 <- renderTable({
    ...
    (data.frame(Y = as.character(as.matrix(forecast_out2)), date = as.character(as.Date(forecast_out2))))
  ...
  output$out3 <- renderTable({
    ...
    (data.frame(Y = as.character(as.matrix(forecast_out3)), date = as.character(as.Date(forecast_out3))))
  })
  ...
  output$out4 <- renderTable({
    ...
    data.frame(Predictions = as.character(FR))
  ...
  output$out6 <- renderTable({
    TotalTS<-ts(MyData()[,Col()],start=Start(),frequency = Fre())
    InsampleTs<-window(TotalTS,start = c(Start(),1),end=c((Hstar()-1),Fre()))
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
    forecast6<-forecast(auto.arima(InsampleTs),level = input$CI5,h=length(OutsampleTs))
    forecast_out6 <- forecast6$lower
    (data.frame(Y = as.character(as.matrix(forecast_out6)), date = as.character(as.Date(forecast_out6))))
  })
